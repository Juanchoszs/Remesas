...
export const buildSiigoPayload = (state: any): SiigoPayload => {
  ...
  if (state.invoiceType === 'purchase') {
    ...
    if (payments.length === 0) {
      const errorMsg = 'No se encontraron métodos de pago válidos. Asegúrese de seleccionar un método de pago y un monto mayor a cero.';
      console.error(errorMsg, { pagos: state.pagos });
    ...
    // Si hay pagos en el estado, usarlos
    if (state.pagos && state.pagos.length > 0) {
      state.pagos.forEach((pago: any) => {
        // Asegurarse de que el ID del método de pago sea un número
        const paymentMethodId = Number(pago.paymentMethodId || pago.cuentaId);
        
        // Validar que el ID del método de pago sea un número válido
        if (!isNaN(paymentMethodId) && pago.value > 0) {
          payments.push({
            id: paymentMethodId,
            value: Number(pago.value || pago.monto || 0),
            due_date: state.invoiceDate || new Date().toISOString().split('T')[0]
          });
        }
      });
    } 
    // Si no hay pagos pero hay un método de pago seleccionado, usarlo
    else if (state.paymentMethod) {
      payments.push({
        id: Number(state.paymentMethod),
        value: total,
        due_date: state.invoiceDate || new Date().toISOString().split('T')[0]
      });
    }
    
    // Si no hay pagos válidos, lanzar un error
    if (payments.length === 0) {
      console.error('No se encontraron pagos válidos');
      throw new Error('Debe agregar al menos un método de pago válido');
    }
